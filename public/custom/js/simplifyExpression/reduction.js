...
function CNFConvert(clauseArray) {
    ...
    whileLoop:
       while (CNFRulesApply(clauseArray)) {
           ...
           console.log("top",JSON.stringify(clauseArray));
               ...
               for (var i=0;i<clauseArray.length;i++) {
                   ...
                   for (var j=0;j<clause.length;j++) {
                       ...
                       if (prop[0] === 'C') {
                           ...
                           clause.splice(j,1,'N'+ops[0],'N'+ops[1]);
                           noteList.push('Phân bố tuyển');
                           noteLightList.push([i,j]);
                           skip = true;
                           break bigloop;
                       } else if (prop[0] === 'K') {
                           ...
                           for (var j=0;j<clauseArray.length;j++) {
                               ...
                               if (clauseArray[i] !== clauseArray[j]) {
                                   if (resolve(clauseArray[i],clauseArray[j]) !== null) {
                                       ...
                                       console.log('before small break',JSON.stringify(clauseArray));
                                       noteList.push('Phần tử bù');
           ...
           console.log('bottom',clauseArray);
       };
    addToResTable(clauseArray);
    return clauseArray;
};
...
function resolve(clause1, clause2) {
    console.log('resolve start','clause1',JSON.stringify(clause1),'clause2',JSON.stringify(clause2));
...
function showResTable() {
    ...
    $(tdSet).each(function(index, element) {
        $(this).delay(d).animate({ fontSize: fontsize, opacity: "1"}, "slow");
        d += 750
        if (noteList.length > 0) {
            ...
            $(newTD1).delay(d).animate({ fontSize: fontsize, opacity: "1"}, "slow");
...
function lightLast() {
    ...
    spans.each(function(index, element) {
        if (element.firstElementChild.innerHTML === ' 0 ') {
            // console.log(element);
